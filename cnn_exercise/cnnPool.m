...
for numImg=1:numImages
  for numFea=1:numFeatures
    for j=1:floor(convolvedDim / poolDim)
      for k=1:floor(convolvedDim / poolDim)
        pval = mean(mean(convolvedFeatures(numFea, numImg, ...
                    (j-1)*poolDim + 1:j*poolDim, 
                    (k-1)*poolDim + 1:k*poolDim)));
                    
        pooledFeatures(numFea, numImg, j, k) = pval;
      end
    end
  end
end
